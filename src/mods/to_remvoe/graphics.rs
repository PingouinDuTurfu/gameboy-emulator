...
use crate::mods::to_remvoe::gpu_memory::{BYTES_PER_PIXEL, GpuMemory, LCDC_REG, OAM_END, OAM_START, STAT_REG, VRAM_END, VRAM_START};

use crate::mods::to_remvoe::ppu;
use crate::mods::to_remvoe::ppu::PpuState;
use crate::mods::to_remvoe::ppu::PpuState::{HBlank, OamSearch, PictureGeneration, VBlank};
    ...
    pub fn adv_cycles(self: &mut Self, io: &mut InputOutput, cycles: usize) {
        ...
        unsafe {
            self.state = match state {
                OamSearch(os) => {
                    PRINT_DEBUG.add_data("OamSearch\n".to_string());
                    os.render(&mut self.gpu_data, cycles)
                },
                PictureGeneration(pg) => {
                    PRINT_DEBUG.add_data("PictureGeneration\n".to_string());
                    pg.render(&mut self.gpu_data, cycles)
                },
                HBlank(hb) => {
                    PRINT_DEBUG.add_data("HBlank\n".to_string());
                    hb.render(&mut self.gpu_data, cycles)
                },
                VBlank(vb) => {
                    PRINT_DEBUG.add_data("VBlank\n".to_string());
