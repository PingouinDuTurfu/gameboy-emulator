...
impl CPU {
    ...
    pub(crate) fn step(&mut self, debug: bool) {
        ...
        if debug {
            unsafe {
                PRINT_DEBUG.add_data(format!("step {} 0x{:04X} ", PRINT_DEBUG.global_index, self.pc));
            }
        }
        ...
        if prefixed {
            if debug {
                unsafe {
                    PRINT_DEBUG.add_data(format!("{}\n", CPU::print_debug_prefixed(instruction_byte)));
                }
            }
            panic!("OOOOO putain c'est la merde");
            // instruction_byte = self.read_pc();
        } else {
            if debug {
                unsafe {
                    PRINT_DEBUG.add_data(format!("{}\n", CPU::print_debug(instruction_byte)));
                }
            }
        }
    ...
    pub fn execute(&mut self, byte: u8) {
        ...
        match byte {
            ...
            0x20 => {
                ...
                unsafe {
                    if PRINT_DEBUG.global_index == 12325 {
    ...
    fn jump_relative(&mut self, should_jump: bool) {
        if should_jump {
            let offset = self.read_pc() as i8 as i16;
            self.pc = self.pc.wrapping_add(offset as u16);
        } else {
            self.pc = self.pc.wrapping_add(1);
        }
    ...
    fn print_debug(byte: u8) -> String {
        unsafe {
            PRINT_DEBUG.add_data(format!("0x{:02X} ", byte));
        }
        match byte {
